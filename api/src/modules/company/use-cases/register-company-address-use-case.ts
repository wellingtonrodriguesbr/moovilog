...
export class RegisterCompanyAddressUseCase {
  ...
  async execute({
    ...
    const state = await this.statesRepository.findByNameAndAcronym(stateName, stateAcronym.toUpperCase());
    ...
    const [city, company] = await Promise.all([
      await this.citiesRepository.findOrCreateByNameAndStateId(cityName, state.id),
