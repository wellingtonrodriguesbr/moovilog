...
export class PrismaCitiesRepository implements CitiesRepository {
  async findOrCreateByNameAndStateId(name: string, stateId: string, tx?: Prisma.TransactionClient) {
  ...
  async findOrCreateManyByStateId(names: string[], stateAcronym: string) {
    const cities = await Promise.all(names.map((name) => this.findOrCreateByNameAndStateId(name, stateAcronym)));
