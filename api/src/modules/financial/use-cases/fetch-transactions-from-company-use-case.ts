...
export class FetchTransactionsFromCompanyUseCase {
  ...
  async execute({
    ...
    const transactions = await this.financeTransactionsRepository.findManyByCompanyId(companyId);

    const totalTransactions = await this.financeTransactionsRepository.countByCompanyId(companyId);

    const totalIncomeInCents = await this.financeTransactionsRepository.sumByTypeAndCompanyId("INCOME", companyId);

    const totalExpenseInCents = await this.financeTransactionsRepository.sumByTypeAndCompanyId("EXPENSE", companyId);

    const totalBalanceInCents = totalIncomeInCents - totalExpenseInCents;

    const percentage =
      totalIncomeInCents !== 0 ? (totalBalanceInCents / totalIncomeInCents) * 100 : totalBalanceInCents < 0 ? -100 : 0;
