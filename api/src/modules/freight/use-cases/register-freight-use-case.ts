import { IFreight, IFreightModality, IFreightTypes } from "@/modules/freight/interfaces/freight";
...
export class RegisterFreightUseCase {
  ...
  async execute({
    ...
    const financeCategory = await this.financeCategoriesRepository.findByName("Coletas e Entregas");

    if (!financeCategory) {
      throw new ResourceNotFoundError("Finance category not found");
    }

    const freight = await this.transactionService.executeTransaction(async (tx) => {
