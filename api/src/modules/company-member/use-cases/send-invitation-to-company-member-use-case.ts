...
export class SendInvitationToCompanyMemberUseCase {
  ...
  async execute({
    ...
    const senderInCompany = await this.companyMembersRepository.findMemberInCompany(senderId, companyId);

    if (!senderInCompany) {
      throw new ResourceNotFoundError("Sender not found in company");
    }

    const hasPermission = await this.permissionService.hasPermission(senderInCompany.id, [
      "SUPER_ADMIN",
      "ADMIN",
      "MANAGE_USERS",
      "MANAGE_PERMISSIONS",
    ...
    if (userAlreadyExists) {
      const memberAlreadyExists = await this.companyMembersRepository.findMemberInCompany(
      ...
      throw new UserAlreadyExistsError("Already exists a user with this same email address");
    }
