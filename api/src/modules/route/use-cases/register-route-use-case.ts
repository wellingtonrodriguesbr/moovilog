...
export class RegisterRouteUseCase {
  ...
  async execute({
    ...
    const routeAlreadyExistsWithSameNameInCompany = await this.routesRepository.findRouteInCompanyWithSameName(
      name,
      member.companyId
    );

    if (routeAlreadyExistsWithSameNameInCompany) {
      throw new BadRequestError("Route already exists with same name in company");
    }
    ...
    const cities = await this.citiesRepository.findOrCreateManyByStateId(cityNames, state.id);
