...
export class FetchCitiesFromRouteUseCase {
  ...
  async execute({ userId, routeId }: FetchCitiesFromRouteUseCaseRequest): Promise<FetchCitiesFromRouteUseCaseResponse> {
    ...
    const citiesInRoute = await this.citiesInRouteRepository.findManyByRouteId(route.id);

    if (!citiesInRoute?.length) {
      throw new ResourceNotFoundError("No cities in route found");
    }

    const cities = await this.citiesRepository.findManyByIds(citiesInRoute.map((city) => city.cityId));
