...
export class UpdateDriverStatusUseCase {
  ...
  async execute({ driverId, userId, status }: UpdateDriverStatusUseCaseRequest): Promise<void> {
    ...
    const driverInCompany = await this.driversRepository.findDriverInCompany(driver.documentNumber, member.companyId);
