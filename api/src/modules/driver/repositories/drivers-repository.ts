...
export interface DriversRepository {
  ...
  findByPhoneNumberInCompany(phone: string, companyId: string): Promise<Driver | null>;
  findDriverInCompany(documentNumber: string, companyId: string): Promise<Driver | null>;
  findById(id: string): Promise<Driver | null>;
  findManyByCompanyId(companyId: string): Promise<Driver[]>;
  updateStatus(id: string, status: AccountStatus): Promise<void>;
  update(driverId: string, data: Prisma.DriverUncheckedUpdateInput): Promise<void>;
}
