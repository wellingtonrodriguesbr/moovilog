...
import { IVehicleBody, IVehicleCategory, IVehicleType } from "@/modules/vehicle/interfaces/vehicle";
...
export class UpdateVehicleUseCase {
  ...
  async execute({
    ...
    if (transformedPlate && transformedPlate !== vehicle.plate) {
      const vehicleAlreadyExistsInCompanyWithPlate = await this.vehiclesRepository.findVehicleInCompanyByPlate(
        transformedPlate,
        member.companyId
      );

      if (vehicleAlreadyExistsInCompanyWithPlate) {
        throw new VehicleAlreadyExistsInCompanyError("There is already a vehicle with this plate");
      }
    }

    const transformedTrailerPlate = trailerPlate?.toUpperCase();

    if (transformedTrailerPlate && transformedTrailerPlate !== vehicle.trailerPlate) {
      const vehicleAlreadyExistsInCompanyWithTrailerPlate =
        await this.vehiclesRepository.findVehicleInCompanyByTrailerPlate(transformedTrailerPlate, member.companyId);

      if (vehicleAlreadyExistsInCompanyWithTrailerPlate) {
        throw new VehicleAlreadyExistsInCompanyError("There is already a vehicle with this trailer plate");
      }
    }
