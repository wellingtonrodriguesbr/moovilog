...
export function useFetchTransactionsFromCompany() {
  ...
  const { data: transactionsFromCompany, isPending: isFetchTransactionsFromCompanyPending } = useQuery({
  ...
  async function handleFetchTransactionsFromCompany() {
    try {
      const { data } = await api.get<TransactionResponse>(`/companies/${company?.id}/transactions`);
