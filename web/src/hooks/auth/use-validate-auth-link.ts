...
export function useValidateAuthLink() {
  ...
  async function handleValidateAuthLink() {
    try {
      const { data } = await api.get<AuthLinkResponse>(`/token/${code}/validate`);

      router.push(redirectLink ? `${redirectLink}?id=${data.userId}` : `/concluir-cadastro?id=${data.userId}`);
    } catch (error) {
      if (error instanceof AxiosError) {
        if (error.response?.status === 400) {
          toast.error("Código expirado ou inválido, solicite um novo.");
        }
