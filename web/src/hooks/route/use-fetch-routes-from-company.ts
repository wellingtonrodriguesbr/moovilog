...
type SimplifiedUser = Omit<CompanyMember["user"], "phone" | "createdAt" | "updatedAt">;
...
export function useFetchRoutesFromCompany() {
  ...
  const { data: routes, isPending: isFetchRoutesFromCompanyPending } = useQuery({
    queryKey: ["routes"],
  ...
  async function handleFetchRoutesFromCompany() {
    const { data } = await api.get<FetchRoutesFromCompanyResponse>(`/companies/${company?.id}/routes`);
