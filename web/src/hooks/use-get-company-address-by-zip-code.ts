...
export function useGetCompanyAddressByZipCode({ zipCode }: UseGetCompanyAddressByZipCodeProps) {
  ...
  const { data, isPending, status } = useQuery({
    ...
    enabled: !!isReady() && !!debouncedValue.length && debouncedValue.length > 5,
  });

  async function handleValidZipCode({ zipCode }: { zipCode: string }) {
    try {
      const { data } = await axios.get<DataResponse>(`https://brasilapi.com.br/api/cep/v2/${zipCode}`);
