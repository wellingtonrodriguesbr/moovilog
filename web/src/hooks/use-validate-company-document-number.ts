...
export function useValidateCompanyDocumentNumber({ documentNumber }: UseValidateCompanyDocumentNumberProps) {
  ...
  const {
    ...
    enabled: !!isReady() && !!debouncedValue.length && debouncedValue.length > 8,
  });

  async function handleValidDocumentNumber({ documentNumber }: { documentNumber: string }) {
    try {
      const { data } = await axios.get<UseValidateCompanyDocumentNumberResponse>(
