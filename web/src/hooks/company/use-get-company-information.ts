...
export function useGetCompanyInformation() {
  ...
  const enabledQuery = !pathName.includes("/cadastro/empresa") && pathName !== "/";

  const { data: companyInformation, isPending: isGetCompanyInformationPending } = useQuery({
  ...
  async function handleGetCompanyInformation() {
    setLoading(true);
    try {
      const { data } = await api.get<CompanyInformationResponse>("/companies/information");
      ...
      if (error instanceof AxiosError) {
        if (
          ...
          error.response.data.message === "Complete the company registration first"
        ) {
        ...
        if (
          error.response?.status === 400 &&
          error.response.data.message === "Complete the company address registration first"
        ) {
