...
import { MisteriousGift } from "@/components/lp/components/misterious-gift";
import { ScrollToTopButton } from "@/components/lp/components/scroll-to-top-button";
      ...
      <div className="w-full max-w-screen-2xl mx-auto px-4 flex flex-col-reverse gap-8 md:gap-4 md:flex-row justify-between">
        <div className="max-w-xl text-white">
          ...
          <h6 className="text-3xl md:text-5xl leading-normal md:leading-tight font-semibold mt-3">
            A solução que faltava para a gestão eficiente da sua transportadora.
          </h6>
          <p className="mt-4 opacity-80">
            Movimento é vida: assim como o corpo precisa se manter ativo para prosperar, a gestão eficiente de um
            negócio depende de processos ágeis e dinâmicos, garantindo crescimento e resultados sustentáveis.
          </p>
          <Button
        ...
        <div>
          <Image src="/logo.svg" alt="moovilog" className="w-[170px]" width={250} height={193} />
          <cite className="text-xs block mt-4 max-w-[10rem] text-zinc-200">O movimento que transforma sua gestão.</cite>
        </div>
      </div>
      ...
      <div className="py-6 mt-12 border-t border-white/15 w-full flex-1 max-w-screen-2xl mx-auto px-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-12">
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm md:text-base flex-1 text-white">
            © Copyright 2024 - Moovilog - Todos os direitos reservados.
          </span>
        </div>
        <FooterSocialMedia />
      </div>
