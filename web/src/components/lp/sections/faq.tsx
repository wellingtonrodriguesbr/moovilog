    ...
    <section className="w-full max-w-screen-2xl px-4 mx-auto pt-12 md:pt-36">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24">
        <div className="flex flex-col gap-3">
          <small className="flex items-center gap-1 mb-2 before:w-8 before:h-[1px] before:bg-app-blue-300">FAQ</small>
          <h4 className="text-2xl md:text-4xl font-semibold">Dúvidas frequentes</h4>
          <p className="text-sm text-app-gray-600 max-w-xl">
            Se ficou com alguma dúvida, entre em contato através do nosso e-mail ou Whatsapp:
          </p>
        ...
        <Accordion type="single" collapsible className="w-full">
          {ITEMS.map(({ value, trigger, content }) => (
            <AccordionItem key={value} value={value}>
              <AccordionTrigger className="underline-0 hover:no-underline">{trigger}</AccordionTrigger>
...
const ITEMS = [
  ...
  {
    value: "5",
    trigger: "Se eu formatar meu computador, eu perco os dados da plataforma?",
    content:
