...
interface EmptyProps {
  context: "vehicle" | "driver" | "route" | "freight" | "pickup" | "financial";
}
  ...
  return (
    <div className="flex items-center justify-center flex-col gap-4 w-full h-auto p-6 md:p-12 rounded-md bg-zinc-100 text-zinc-600">
      <div className="w-full h-full max-w-[300px] max-h-[300px] pointer-events-none">
        <Lottie options={DEFAULT_OPTIONS} style={{ objectFit: "contain", pointerEvents: "none" }} />
      </div>
      <h6 className="text-center text-2xl text-app-blue-900">Nenhum dado encontrado!</h6>
      <p className="text-center max-w-full md:max-w-sm">
        {EMPTY_CONTEXTS[context]}, clique no botão superior direito e cadastre agora mesmo.
      </p>
    </div>
  );
}
