...
import { Building, Building2, CircleHelp, Globe, Loader2, LogOut, MessageSquareMore, User } from "lucide-react";
  ...
  return (
    <Dialog>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="min-w-56 z-[999999]">
          <DropdownMenuLabel className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <span className="block text-nowrap text-ellipsis max-w-[170px] overflow-hidden">{profile?.name}</span>
          ...
          <DropdownMenuItem className="hover:bg-zinc-100" asChild>
            <Link className="gap-2 cursor-pointer" href="/meu-perfil">
              <User className="size-4" />
              Meu perfil
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem className="hover:bg-zinc-100" asChild>
            <Link className="gap-2 cursor-pointer" href="/dados-cadastrais">
              <Building className="size-4" />
              Dados da empresa
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem className="hover:bg-zinc-100" asChild>
            <Link className="gap-2 cursor-pointer" href="/" target="_blank">
              <Globe className="size-4" />
              Ver página inicial do site
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem className="flex xl:hidden hover:bg-zinc-100" asChild>
            <Link className="gap-2 cursor-pointer" href={links["send-feedback"]} target="_blank">
              <MessageSquareMore className="size-4" />
              Enviar feedback
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem className="flex xl:hidden hover:bg-zinc-100" asChild>
            <Link className="gap-2 cursor-pointer" href={links["contact-us"]}>
          ...
          >
            <button className="w-full" onClick={handleLogout}>
              {isPendingLogout ? <Loader2 className="mr-2 size-4 animate-spin" /> : <LogOut className="mr-2 size-4" />}
              <span>Sair</span>
            </button>
          </DropdownMenuItem>
        </DropdownMenuContent>
