...
import { FetchCitiesFromStateResponse, useFetchCitiesFromState } from "@/hooks/use-fetch-cities-from-state";
...
export function RegisterRouteForm({ onCloseDialog }: RegisterRouteFormProps) {
  ...
  const { registerNewRoute, isPendingRegisterNewRoute } = useRegisterNewRoute();
      ...
      >
              ...
              <Select onValueChange={field.onChange}>
                ...
                <SelectContent className="max-h-[300px]">
                  {STATES_ARRAY.map((state) => (
                    <SelectItem value={state.acronym} key={state.acronym}>
                      {state.name}
                    </SelectItem>
                  ))}
                </SelectContent>
        ...
        <fieldset className="space-y-2">
          ...
          <MultiSelectCities
            ...
            disabled={!form.watch("stateAcronym") || isFetchCitiesFromStatePending}
            placeholder={
              form.watch("stateAcronym") && isFetchCitiesFromStatePending
                ? "Carregando cidades.."
        ...
        <fieldset className="flex justify-end gap-2 mt-6">
          <Button onClick={onCloseDialog} type="button" variant="destructive-outline">
            Cancelar
          </Button>
          <Button
            disabled={!form.watch("cityNames").length || isPendingRegisterNewRoute || isFetchCitiesFromStatePending}
            type="submit"
          >
            {isPendingRegisterNewRoute ? <Loader2 className="size-4 animate-spin" /> : "Cadastrar"}
          </Button>
        </fieldset>
      </form>
    </Form>
