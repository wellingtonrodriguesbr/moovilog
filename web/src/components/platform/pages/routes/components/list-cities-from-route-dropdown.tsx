...
export function ListCitiesFromRouteDropdown({ routeId }: ListCitiesFromRouteDropdownProps) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent>
        ...
        <DropdownMenuSeparator />
        {cities?.map((city) => <DropdownMenuItem key={city.id}>{city.name}</DropdownMenuItem>)}
      </DropdownMenuContent>
    </DropdownMenu>
  );
}
