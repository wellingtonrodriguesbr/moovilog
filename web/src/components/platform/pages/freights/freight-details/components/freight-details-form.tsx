...
export function FreightDetailsForm() {
  const { freightDetails, isGetFreightDetailsPending } = useGetFreightdetailsFromCompany();
    ...
    <Card className="bg-white border p-6">
      <form className="space-y-6">
        <fieldset className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Label className="flex flex-col gap-2 font-normal">
            Valor do frete:
            <Input value={formatCurrencyBR(freightDetails.freightAmountInCents)} />
          </Label>
        </fieldset>
        <fieldset className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Label className="flex flex-col gap-2 font-normal">
        ...
        <fieldset className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <Label className="flex flex-col gap-2 font-normal">
            ...
            <Input value={`${formatWeight(freightDetails.totalWeightOfDeliveries)}kg`} />
          </Label>
          <Label className="flex flex-col gap-2 font-normal">
            Quantidade de coletas:
            <Input value={freightDetails.pickupsQuantity > 0 ? freightDetails.pickupsQuantity : "-"} />
          </Label>
          <Label className="flex flex-col gap-2 font-normal">
            Peso total de coletas:
            <Input
              value={
                freightDetails.totalWeightOfPickups ? `${formatWeight(freightDetails.totalWeightOfPickups)}kg` : "-"
              }
            />
          </Label>
          <Label className="flex flex-col gap-2 font-normal col-span-2 md:col-span-1">
            Cadastro em:
            <Input value={formatBrazilianDate(freightDetails.createdAt)} />
          </Label>
        </fieldset>
        <Label className="flex flex-col gap-2 font-normal">
          Observação:
