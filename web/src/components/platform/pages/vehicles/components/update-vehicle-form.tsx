...
const formSchema = z.object({
  plate: z.string({ message: "Digite a placa do veículo" }).regex(PLATE_REGEX, {
...
export function UpdateVehicleForm({ onCloseDialog, vehicle }: UpdateDriverFormProps) {
  ...
  const shouldIncludeTrailerPlate = typesThatRequireTrailerPlate.includes(form.watch("category"));
      ...
      >
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Ex: ATEGO 1419" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent className="max-h-[300px]">
                    {VEHICLE_BODIES.map((body) => (
                      <SelectItem key={body.label} value={body.value}>
                        {body.label}
                      </SelectItem>
                    ))}
                  </SelectContent>
        ...
        >
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{shouldIncludeTrailerPlate ? "Placa do cavalo" : "Placa"}</FormLabel>
                <FormControl>
                  <Input
        ...
        <fieldset className="flex justify-end gap-2 mt-6">
          <Button disabled={isPendingUpdateVehicle} onClick={onCloseDialog} type="button" variant="destructive-outline">
            Cancelar
          </Button>
          <Button disabled={isPendingUpdateVehicle} type="submit">
            {isPendingUpdateVehicle ? <Loader2 className="size-4 animate-spin" /> : "Salvar"}
          </Button>
        </fieldset>
      </form>
    </Form>
