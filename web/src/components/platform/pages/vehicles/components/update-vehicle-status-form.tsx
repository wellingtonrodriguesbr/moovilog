...
export function UpdateVehicleStatusForm({ onCloseDialog, vehicleId, currentStatus }: UpdateVehicleStatusFormProps) {
  const { updateVehicleStatus, isUpdateVehicleStatusPending } = useUpdateVehicleStatus();
    ...
    <Form {...form}>
      ...
      >
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent>
                  {VEHICLE_STATUS.map((status) => (
                    <SelectItem key={status.key} value={status.key} disabled={status.key === currentStatus}>
                      {status.label}
                    </SelectItem>
                  ))}
                </SelectContent>
        ...
        <fieldset className="flex justify-end gap-2 mt-6">
          ...
          <Button disabled={isUpdateVehicleStatusPending} type="submit">
            {isUpdateVehicleStatusPending ? <Loader2 className="size-4 animate-spin" /> : "Salvar"}
          </Button>
        </fieldset>
      </form>
    </Form>
