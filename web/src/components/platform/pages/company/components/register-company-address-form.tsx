...
export function RegisterCompanyAddressForm() {
  ...
  const { registerCompanyAddress, isPendingRegisterCompanyAddress } = useRegisterCompanyAddress();
  ...
  const { data, isPendingGetCompanyAddress, status } = useGetCompanyAddressByZipCode({
    zipCode,
  });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4 w-full">
              ...
              <FormControl>
                <div className="flex items-center rounded-md overflow-hidden border pr-4 bg-white">
                  <Input
                    ...
                    {...field}
                    ref={inputRef}
                    onChange={({ currentTarget }) => form.setValue("zipCode", currentTarget.value.replace(/\D/g, ""))}
                    value={formatCEP(field.value)}
                  />
                  {zipCode.length === 8 && isPendingGetCompanyAddress ? (
                    <Loader2 className="size-4 text-app-blue-500 animate-spin" />
                  ) : null}
                  {!isPendingGetCompanyAddress && status === "error" ? <X className="size-4 text-red-500" /> : null}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="col-span-3">
                ...
                <FormControl>
                  <Input placeholder="" disabled={isPendingGetCompanyAddress || status === "pending"} {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
              ...
              <FormItem>
                ...
                <FormControl>
                  <Input
                    placeholder=""
                    type="number"
                    disabled={isPendingGetCompanyAddress || status === "pending"}
                    {...field}
                  />
                </FormControl>
                <FormMessage />
        ...
        <Button
          disabled={isPendingRegisterCompanyAddress || isPendingGetCompanyAddress}
          type="submit"
