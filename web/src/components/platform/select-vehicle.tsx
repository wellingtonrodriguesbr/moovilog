...
export function SelectVehicle({ selectedVehicle, onChangeSelectedVehicle }: SelectVehicleProps) {
  ...
  function handleSelectVehicle(currentValue: string) {
    const selectedVehicleObj = vehiclesFromCompany.find((vehicle) => vehicle.plate === currentValue);

    if (selectedVehicleObj) {
      onChangeSelectedVehicle(selectedVehicleObj.id === selectedVehicle ? "" : selectedVehicleObj.id);
    }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {selectedVehicle
            ? vehiclesFromCompany.find((vehicle) => vehicle.id === selectedVehicle)?.plate
            : "Selecione o veículo"}
            ...
            <CommandGroup>
              {vehiclesFromCompany.map((vehicle) => (
                <CommandItem
                  ...
                  onSelect={(currentValue) => handleSelectVehicle(currentValue)}
                >
                  <Check
                    className={cn(
