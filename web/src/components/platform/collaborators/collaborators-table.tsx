    ...
    <Table>
            ...
            <Tooltip defaultOpen={true}>
              ...
              <TooltipContent className="max-w-64">
                <p className="font-normal text-zinc-700">
                  O <strong className="underline">cargo</strong> refere-se as
                  permissões que este usuário poderá realizar dentro da conta da
                  empresa.
                </p>
              </TooltipContent>
            </Tooltip>
      ...
      <TableBody>
        {companyMembers.map((companyMember) => (
          <TableRow className="hover:bg-transparent" key={companyMember.id}>
            <TableCell className="text-nowrap">
              {companyMember.member.name}
            </TableCell>
            <TableCell>
              <ButtonToCopyEmail email={companyMember.member.email} />
            </TableCell>
            <TableCell>-</TableCell>
            <TableCell>{COMPANY_MEMBER_ROLE[companyMember.role]}</TableCell>
            <TableCell>
              {dayjs(companyMember.createdAt).format("DD/MM/YYYY")}
            </TableCell>
