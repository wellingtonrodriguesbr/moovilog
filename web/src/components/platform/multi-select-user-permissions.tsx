 ...
 */
const multiSelectUserPermissionsVariants = cva("m-1 text-app-blue-900", {
  variants: {
    variant: {
...
/**
 * Props for MultiSelectUserPermissions component
 */
interface MultiSelectUserPermissionsProps
  extends React.ButtonHTMLAttributes<HTMLButtonElement>,
    VariantProps<typeof multiSelectUserPermissionsVariants> {
  options: {
    key: string;
    label: string;
    icon?: React.ComponentType<{ className?: string }>;
  }[];
  onUserPermissionsChange: (name: string[]) => void;
...
export const MultiSelectUserPermissions = React.forwardRef<HTMLButtonElement, MultiSelectUserPermissionsProps>(
  (
    {
      options,
    ...
    const handleInputKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
      if (event.key === "Enter") {
        ...
        onUserPermissionsChange(newSelectedValues);
      }
    };
    ...
    const toggleAll = () => {
      if (selectedValues.length === options.length) {
        ...
        const allNames = options.map((option) => option.key);
        setSelectedValues(allNames);
        onUserPermissionsChange(allNames);
      }
    };
            ...
            {selectedValues.length > 0 ? (
              <div className="flex justify-between items-center w-full">
                <div className="flex flex-wrap items-center">
                  {selectedValues.slice(0, maxCount).map((key) => {
                    ...
                    return (
                      <Badge
                        key={key}
                        className={cn(
                          multiSelectUserPermissionsVariants({
                            variant,
                          })
                        )}
                      >
            ...
            <CommandList>
              ...
              <CommandGroup>
                ...
                {options.map((option) => {
                  ...
                  return (
                    <CommandItem key={option.key} onSelect={() => toggleOption(option.key)} className="cursor-pointer">
                      <div
                        className={cn(
                          "mr-2 flex p-0.5 items-center justify-center rounded-md border",
...
MultiSelectUserPermissions.displayName = "MultiSelectUserPermissions";
