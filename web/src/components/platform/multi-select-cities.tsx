...
export const MultiSelectCities = React.forwardRef<HTMLButtonElement, MultiSelectCitiesProps>(
  (
    {
      options,
      onCitiesChange,
            ...
            {selectedValues.length > 0 ? (
              <div className="flex justify-between items-center w-full">
                <div className="flex flex-wrap items-center">
                  {selectedValues.slice(0, maxCount).map((nome) => {
                    const option = options.find((o) => o.nome === nome);
                    return (
                      <Badge
                        key={nome}
                        className={cn(
          ...
          <Command>
            ...
            <CommandList>
              ...
              <CommandGroup>
                ...
                {options.map((option) => {
                  const isSelected = selectedValues.includes(option.nome);
                  return (
                    <CommandItem key={option.id} onSelect={() => toggleOption(option.nome)} className="cursor-pointer">
                      <div
                        className={cn(
                          "mr-2 flex p-0.5 items-center justify-center rounded-md border",
