...
export function EnterYourEmailForm() {
  ...
  const { sendEmailToResetPassword, isPendingSendEmailToResetPassword } = useSendEmailToResetPassword();
    ...
    <>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4 w-full">
          ...
          <Button disabled={isPendingSendEmailToResetPassword} type="submit" className="w-full">
            {isPendingSendEmailToResetPassword ? <Loader2 className="size-4 animate-spin" /> : "Salvar"}
          </Button>
        </form>
      </Form>
      <div className="flex justify-center items-center mt-4">
        <Link href="/entrar" className="w-fit text-center">
          Lembrou da senha? <span className="underline font-medium">Faça login</span>
        </Link>
      </div>
    </>
  );
